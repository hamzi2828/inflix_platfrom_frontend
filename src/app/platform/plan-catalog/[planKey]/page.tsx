...
import { ArrowLeft, Loader2, Save, Pencil, DollarSign, ToggleLeft, Gauge, Sparkles } from "lucide-react";
import {
  platformApi,
...
export default function EditPlanPage() {
  ...
  const loadData = useCallback(() => {
    Promise.all([
      ...
      .then(([plans, f, l]) => {
        ...
        const pm = found.priceMetadata && typeof found.priceMetadata === "object" ? found.priceMetadata as { monthly?: number; yearly?: number; currency?: string } : null;
        setMonthly(pm?.monthly ?? "");
        setYearly(pm?.yearly ?? "");
    ...
    try {
      const priceMetadata =
        monthly !== "" || yearly !== ""
          ...
          : null;
      await platformApi.updatePlan(planKey, {
        name,
        description,
        priceMetadata,
        features,
  ...
  if (notFound || !plan) {
    return (
      <div className="max-w-xl mx-auto mt-12 p-6 bg-white rounded-xl border border-gray-200 shadow-sm text-center">
        <p className="text-gray-700 mb-4">Plan &quot;{planKey}&quot; not found.</p>
        <Link href="/platform/plan-catalog" className="text-orange-600 hover:underline">Back to Plan Catalog</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Edit Plan</h1>
          <p className="text-sm text-gray-500">
            Editing <span className="font-mono font-medium text-gray-700">{planKey}</span>
          </p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={planKey}
                  disabled
                  className="border border-gray-300 rounded-lg px-3 py-2 w-full font-mono text-sm bg-gray-50 text-gray-500 cursor-not-allowed"
                />
